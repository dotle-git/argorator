...
class AnalysisContext(BaseContext):
    ...
    script_path: Optional[Path] = Field(
        default=None, description="Path to the script file"
    )
    command: str = Field(
        default="", description="The command to execute (run/compile/export)"
    )

    # OUTPUTS: What analysis produces
    shell_cmd: List[str] = Field(
        default_factory=list, description="Shell command for execution"
    )
    all_used_vars: Set[str] = Field(
        default_factory=set, description="All variables referenced in script"
    )
    defined_vars: Set[str] = Field(
        default_factory=set, description="Variables defined within script"
    )
    loop_vars: Set[str] = Field(
        default_factory=set, description="Variables defined in loop constructs"
    )
    undefined_vars: Dict[str, Optional[str]] = Field(
        default_factory=dict, description="Variables not defined in script"
    )
    env_vars: Dict[str, str] = Field(
        default_factory=dict, description="Variables with environment defaults"
    )
    positional_indices: Set[int] = Field(
        default_factory=set, description="Positional parameter indices used"
    )
    varargs: bool = Field(
        default=False, description="Whether script uses varargs ($@ or $*)"
    )
    annotations: Dict[str, ArgumentAnnotation] = Field(
        default_factory=dict, description="Parsed annotations"
    )
    script_metadata: Optional[ScriptMetadata] = Field(
        default=None, description="Script-level metadata from comments"
    )

    # Temporary data for pipeline steps
    temp_data: Dict[str, Any] = Field(
        default_factory=dict, description="Temporary data for pipeline steps"
    )
...
class TransformContext(BaseContext):
    ...
    argument_parser: Optional[argparse.ArgumentParser] = Field(
        default=None, description="Built argument parser"
    )
...
class ValidateContext(BaseContext):
    ...
    parsed_args: Optional[argparse.Namespace] = Field(
        default=None, description="Parsed command line arguments"
    )
...
class CompileContext(BaseContext):
    ...
    annotations: Dict[str, ArgumentAnnotation] = Field(
        default_factory=dict, description="Parsed argument annotations"
    )

    # OUTPUTS: What compile produces
    compiled_script: str = Field(
        default="", description="Compiled script with injected variables"
    )
    variable_assignments: Dict[str, str] = Field(
        default_factory=dict, description="Resolved variable assignments"
    )
    positional_values: List[str] = Field(
        default_factory=list, description="Positional argument values"
    )
...
def create_validate_context(
    transform: TransformContext, parsed_args: argparse.Namespace
) -> ValidateContext:
    """Create a ValidateContext from TransformContext and parsed args."""
    return ValidateContext(
        argument_parser=transform.argument_parser, parsed_args=parsed_args
    )


def create_compile_context(
    analysis: AnalysisContext, validate: ValidateContext, echo_mode: bool
) -> CompileContext:
...
def create_execute_context(
    analysis: AnalysisContext, compile_ctx: CompileContext
) -> ExecuteContext:
