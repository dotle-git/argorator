...
def create_base_parser(context: TransformContext) -> None:
            ...
            if conflicts:
                ...
                for var_name, env_val, ann_val in conflicts:
                    warning_lines.append(
                        f"  {var_name}: environment='{env_val}' vs annotation='{ann_val}' (using environment)"
                    )
                warning_lines.append("")
                help_text = help_text + "\n".join(warning_lines)
            return help_text
    ...
    parser = ConflictAwareArgumentParser(
        add_help=True, prog=context.get_script_name(), description=description
    )
...
def build_top_level_parser(
    script_name: str = "argorator", script_description: Optional[str] = None
    ...
    description = (
        script_description
        or "Execute or compile shell scripts with CLI-exposed variables"
    )
    ...
    run_parser = subparsers.add_parser(
        "run", help="Run script (default)", description=script_description
    )
    run_parser.add_argument("script", help="Path to the shell script")
    run_parser.add_argument(
        "--echo",
        action="store_true",
        help="Print commands that would run (no execution)",
    )

    # compile
    compile_parser = subparsers.add_parser(
        "compile", help="Print modified script", description=script_description
    )
    compile_parser.add_argument("script", help="Path to the shell script")
    compile_parser.add_argument(
        "--echo",
        action="store_true",
        help="Print echo-transformed script (dry run view)",
    )

    # export
    export_parser = subparsers.add_parser(
        "export", help="Print export lines", description=script_description
    )
    export_parser.add_argument("script", help="Path to the shell script")

    # explain
    explain_parser = subparsers.add_parser(
        "explain", help="Print script interface as JSON", description=script_description
    )
