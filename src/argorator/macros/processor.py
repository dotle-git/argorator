...
class MacroProcessor:
    ...
    def process_macros(self, script_text: str) -> str:
        ...
        for comment in macro_comments:
            if comment.macro_type == "iteration":
                target = self.parser.find_target_for_macro(
                    script_text, comment.line_number
                )
                if target:
                    try:
                        iteration_macro = self.parser.parse_iteration_macro(
                            comment, target
                        )
                        ...
                        print(
                            f"Warning: Failed to parse macro on line {comment.line_number + 1}: {e}"
                        )
    ...
    def _apply_transformations(
        self, script_text: str, macros: List[IterationMacro]
    ) -> str:
    ...
    def _apply_single_transformation(
        self, lines: List[str], macro: IterationMacro, transformation: str
    ) -> List[str]:
    ...
    def _generate_nested_loop_alternatives(
        self, macros: List[IterationMacro], target_line: int
    ) -> str:
        ...
        if len(macros) == 2 and all(
            m.iteration_type in ["pattern", "array"] for m in macros
        ):
            alternatives.append(
    ...
    def _generate_function_conflict_alternatives(
        self, func_macro: IterationMacro, internal_macros: List[IterationMacro]
    ) -> str:
    ...
    def _validate_macro_combinations(self, macros: List[IterationMacro]) -> None:
        ...
        for target_line, line_macros in target_groups.items():
            if len(line_macros) > 1:
                ...
                alternatives = self._generate_nested_loop_alternatives(
                    line_macros, target_line
                )
        ...
        for func_macro in function_macros:
            ...
            if conflicting_lines:
                ...
                alternatives = self._generate_function_conflict_alternatives(
                    func_macro, conflicting_lines
                )
        ...
        for comment in macro_comments:
            if comment.macro_type == "iteration":
                try:
                    ...
                    if not target:
                        errors.append(
                            f"Line {comment.line_number + 1}: No target found for macro"
                        )
                        continue
                    ...
                    errors.append(
                        f"❌ INVALID MACRO SYNTAX (Line {comment.line_number + 1})\n{error_details}"
                    )
                except Exception as e:
                    errors.append(
                        f"Line {comment.line_number + 1}: Unexpected error: {e}"
                    )
    ...
    def list_macros(self, script_text: str) -> List[Dict]:
        ...
        for comment in macro_comments:
            ...
            macro_info = {
                ...
                "target_lines": (
                    f"{target.start_line + 1}-{target.end_line + 1}"
                    if target
                    else "none"
                ),
            }
