...
class MacroParser:
    ...
    def find_target_for_macro(
        self, script_text: str, macro_line: int
    ...
    def parse_iteration_macro(
        self, comment: MacroComment, target: MacroTarget
    ) -> IterationMacro:
        ...
        paren_pattern = (
            r"for\s+(\w+)\s+in\s+\(([^)]+?)\s+as\s+(\w+)\)\s*(?:\|\s*with\s+(.+))?"
        )
        ...
        if not self._is_valid_bash_variable_name(iterator_var):
            raise ValueError(
                f"Invalid variable name '{iterator_var}'. Bash variables must start with a letter or underscore, followed by letters, numbers, or underscores."
            )
