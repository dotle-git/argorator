...
class PipelineCommand:
    ...
    def __init__(
        ...
        rest_args: Optional[List[str]] = None,
    ):
...
class Pipeline:
    ...
    def parse_arguments(
        self, transform: TransformContext, rest_args: List[str]
    ) -> argparse.Namespace:
    ...
    def run_validation_stage(
        self, transform: TransformContext, parsed_args: argparse.Namespace
    ) -> ValidateContext:
    ...
    def run_compilation_stage(
        self, analysis: AnalysisContext, validate: ValidateContext, echo_mode: bool
    ) -> CompileContext:
    ...
    def run_execution_stage(
        self, analysis: AnalysisContext, compile_ctx: CompileContext
    ) -> ExecuteContext:
    ...
    def _generate_explain_output(self, context: AnalysisContext) -> str:
        ...
        for index in sorted(context.positional_indices):
            positional_info = PositionalInfo(name=f"ARG{index}", index=index)
            positionals.append(positional_info)

        # Create the script interface
        script_interface = ScriptInterface(
            description=(
                context.script_metadata.description if context.script_metadata else None
            ),
            arguments=arguments,
    ...
    def run(self, command: PipelineCommand) -> int:
        ...
        try:
            ...
            compile_ctx = self.run_compilation_stage(
                analysis, validate, command.echo_mode
            )
