...
def test_google_annotations_with_argparse():
    ...
    parser = build_test_parser(["PORT", "HOST", "DEBUG"], {}, set(), False, annotations)
...
def test_mixed_required_optional():
    ...
    annotations = {
        "SERVICE": ArgumentAnnotation(
            type="str", help="Service name"
        ),  # No default = required
        "PORT": ArgumentAnnotation(
            type="int", help="Port", default="8080"
        ),  # Has default = optional
    }

    parser = build_test_parser(["SERVICE", "PORT"], {}, set(), False, annotations)
...
"""
    ...
    result = cli.main(
        ["compile", str(script_path), "--service", "api", "--environment", "dev"]
    )
