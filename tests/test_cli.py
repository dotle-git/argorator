...
from argorator.analyzers import (
    parse_defined_variables,
    parse_variable_usages,
    parse_positional_usages,
)
...
""",
    ...
    rc = cli.main(
        ["compile", str(script), "--echo"]
    )  # no variables needed due to default
    assert rc == 0


def test_run_echo_does_not_execute_commands(
...
def test_env_annotation_default_conflicts(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys
):
    ...
    monkeypatch.setenv(
        "HOST", "localhost"
    )  # This will match annotation default (no conflict)
...
"""
    ...
    assert (
        "PORT: environment='3000' vs annotation='8080' (using environment)"
        in captured.out
    )
    ...
    assert (
        "(default from env: localhost)" in captured.out
    )  # No override notice for HOST


def test_env_annotation_conflict_execution(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys
):
...
def test_no_conflict_when_no_annotation_default(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys
):
...
def test_multiple_conflicts_in_warning(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys
):
...
"""
    ...
    assert (
        "TIMEOUT: environment='30' vs annotation='60' (using environment)"
        in captured.out
    )
    assert (
        "DEBUG: environment='true' vs annotation='false' (using environment)"
        in captured.out
    )
...
def test_lowercase_annotations_work_with_uppercase_vars(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
):
