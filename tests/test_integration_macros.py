...
class TestIterationMacroIntegration:
    ...
    def _run_argorator(
        self, script_content: str, args: list, expect_success: bool = True
    ):
...
"""
        ...
        output = self._run_argorator(
            script, ["--servers", "web1,web2", "--ports", "80:443"]
        )
...
"""
        ...
        output = self._run_argorator(script, ["--data1", "a,b,c", "--data2", "x::y::z"])
